...
- **Permissions:** 투자 데이터 제공 또는 검토. 직접 시스템 사용자 여부는 `Needs Verification`.
- **Key scenarios:**
  - 분기별 예상 수익 갱신
  - 주말 미사용 자금 투자 보고
...
| ID | Issue | Status |
|---|---|---|
| OI-STK-001 | 투자 고문이 직접 시스템을 사용하는지, 관리자에게 데이터를 전달하는지만 하는지 미정 | Needs Verification |
| OI-STK-002 | 부부/신청자가 직접 시스템을 사용하는지 미정 | Needs Verification |
| OI-STK-003 | 파일럿의 사용자 인증 방식과 사용자 수 미정 | Needs Verification |
